...
async function fetchAndAdaptEvents(communityId: string, status: string): Promise<DisplayEvent[]> {
  const response = await apiClient.get<{ events: EventItem[] }>(`/events/by-community/${communityId}`, {
    params: { status },
  });
...
export default function CommunityEventsTab({ community, initialEvents, currentUser }: CommunityEventsTabProps) {
  ...
  const tabLabelMap = {
    upcoming: t('upcoming_tab'),
    ongoing: t('ongoing_tab'),
    past: t('past_tab'),
  } as const;

  const activeTabLabel = tabLabelMap[activeTab as keyof typeof tabLabelMap] ?? activeTab;
  ...
  const renderContent = () => {
    ...
    if (error) return <div className="text-destructive text-center py-10">{t('error_loading', { message: error.message })}</div>;
    if (!filteredEvents || filteredEvents.length === 0) {
      return (
        <div className="text-center py-16 px-6 rounded-2xl border-2 border-dashed">
          <Calendar className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
          <h3 className="text-lg font-semibold">{t('no_events_found', { activeTab: activeTabLabel })}</h3>
          <p className="mt-1 text-sm text-muted-foreground">
            {searchTerm
              ? t('search_no_results', { searchTerm })
              : t('no_events_yet', { activeTab: activeTabLabel })}
          </p>
        </div>
      );
    }
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-3 glass-card">
          <TabsTrigger value="upcoming" className="liquid-glass-button">{tabLabelMap.upcoming}</TabsTrigger>
          <TabsTrigger value="ongoing" className="liquid-glass-button">{tabLabelMap.ongoing}</TabsTrigger>
          <TabsTrigger value="past" className="liquid-glass-button">{tabLabelMap.past}</TabsTrigger>
        </TabsList>
