...
const EventStatusButton = ({ status, isRegistered }: { status: DisplayEvent['status'], isRegistered: boolean }) => {
  if (status === 'past') {
    return <Button variant="outline" size="sm" className="cursor-default bg-transparent border-white/20 text-white/80 hover:bg-white/10">Ended</Button>;
  }
  if (isRegistered) {
    return <Button variant="secondary" size="sm" className="bg-green-500/20 text-green-300 border-green-500/30 hover:bg-green-500/30">Registered</Button>;
  }
  return <Button variant="default" size="sm">Register</Button>;
};
  ...
  return (
    <Link href={targetHref} passHref className="group">
      <GlassCard
        effect="magnify"
        className={cn(
          "p-0 flex flex-col items-start overflow-hidden relative min-h-[250px]", // Increased min-height
          "border-glass-border bg-transparent", // Make base background transparent
          className
        )}
      >
        {/* Background Image */}
        ...
        <div className="relative z-10 p-6 flex flex-col justify-end h-full w-full">
          {/* Date Section */}
          <div className="absolute top-4 left-4 flex flex-col items-center justify-center w-16 h-16 rounded-lg bg-white/10 border border-white/20 text-white backdrop-blur-sm">
