...
export default function EventSidebarActions({
  ...
  const actionState = useMemo(
    () => determineActionState(event, user, isHost, myRegistration, selectedSession),
    [event, user, isHost, myRegistration, selectedSession]
  );
  ...
  const renderActionComponent = () => {
    switch (actionState.state) {
      ...
      case 'CAN_REGISTER':
        return <RegisterButton eventId={event.id} />;
      case 'EVENT_ENDED':
      case 'REGISTRATION_CLOSED':
      case 'EVENT_FULL':
      case 'PENDING_APPROVAL':
      case 'ATTENDED':
        return <StatusDisplay state={actionState.state} message={'message' in actionState ? actionState.message : undefined} />;
  ...
  return (
    <div className="space-y-4">
      {event.max_attendees?.Valid && (
        <CapacityIndicator current={event.current_attendees} max={maxAttendees} />
      )}
